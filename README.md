...
The project is architected for dual cloud deployment:
- **Backend (FastAPI + Whisper + GPT-4)**: Deployed on **Railway** using the provided `Dockerfile`.
- **Frontend (Expo React Native Web)**: Deployed on **Vercel** as a high-performance static web app.

### 1. Deploy the Backend to Railway

1. **Push your code to GitHub**:
   Ensure all changes in this repo are pushed to your GitHub repository.
2. **Log into Railway**:
   Go to [railway.app](https://railway.app) and sign in with GitHub.
3. **Create a New Project**:
   - Click **New Project** → **Deploy from GitHub repo**.
   - Select your `TranscriptionApp` repository.
4. **Configure the Service Settings**:
   - Click on the newly created service and open the **Settings** tab.
   - Under **Root Directory**, set it to `/` (or `transcription-backend`).
...
2. **Import Project**:
   ...
   - Import your `TranscriptionApp` repository.
3. **Configure Build Settings**:
   - **Framework Preset**: Other (handled automatically by `vercel.json`).
   - **Build Command**: `npm run build` (or `npx expo export -p web`).
   - **Output Directory**: `dist`.
   - **Install Command**: `npm install --legacy-peer-deps` (also pre-configured in `.npmrc` and `vercel.json`).
4. **Add Environment Variable**:
   - In the **Environment Variables** section, add:
     - **Key**: `EXPO_PUBLIC_API_URL`
     - **Value**: `https://your-backend.up.railway.app` *(use the exact domain generated by Railway, without a trailing slash)*.
5. **Click Deploy**:
   Vercel will install dependencies, bundle the web application, and deploy it to a live global URL (e.g. `https://your-app.vercel.app`).
